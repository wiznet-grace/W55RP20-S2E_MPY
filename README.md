# W55RP20-S2E Driver Technical Manual
...
| :--- | :--- | :--- |
| **`at_cmd.py`** | 실행 진입점<br>(Entry Point) | • **모드 선택:** `MODE` 변수로 SPI/UART 로드<br>• **루프 제어:** 무한 루프 내 AT 커맨드 전송<br>• **오류 복구:** `try-except`로 런타임 에러 방어 |
...
### 3. 안정성 및 메모리 관리 (Stability & Memory)
...
| **통신 오류** | 노이즈로 인한<br>시스템 멈춤 | • **Fault Tolerance:** `main.py`의 `try-except` 블록이<br>에러를 포착하고 자동으로 재시도 (Auto-retry) |
...
| File | Role | Key Logic |
| :--- | :--- | :--- |
| **`at_cmd.py`** | Entry Point | • **Init:** Loads SPI/UART driver based on `MODE`<br>• **Loop:** Sends AT commands in `while` loop<br>• **Safety:** Catches runtime errors via `try-except` |
...
| **Comm Error** | System freeze caused<br>by noise or timeout | • **Fault Tolerance:** `try-except` block in `main.py`<br>catches errors and triggers auto-retry |